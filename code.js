// AUTO-GENERATED FILE. Do not edit directly. Source: src/exporter.ts
var SelectionStyleExporterBundle = (() => {
  ...
  var require_exporter = __commonJS({
    "src/exporter.ts"(exports, module) {
      (function(root, factory) {
        ...
        function safeValue(value, seen) {
          ...
          if (Array.isArray(value)) {
            const arrayValue = value.map((item) => safeValue(item, seen)).filter((item) => item !== void 0);
            seen.delete(value);
            return arrayValue;
          }
          const result = {};
        ...
        function readField(node, field, seen) {
          ...
          try {
            return safeValue(node[field], seen || /* @__PURE__ */ new WeakSet());
          } catch (error) {
            return {
        ...
        function createExportContext(options) {
          const maxConcurrency = options && typeof options.maxConcurrency === "number" ? Math.max(1, Math.floor(options.maxConcurrency)) : 8;
          return {
            maxConcurrency,
        ...
        function readObjectProperty(object, property) {
          try {
            return safeValue(object[property], /* @__PURE__ */ new WeakSet());
          } catch (error) {
            return {
              error: error instanceof Error ? error.message : String(error)
            };
        ...
        function cloneDocumentResources(resources) {
          ...
          return safeValue(resources, /* @__PURE__ */ new WeakSet());
        }
        ...
        async function serializeNode(node, context, options, exportContext) {
          ...
          const path = Array.isArray(nodeContext.path) ? nodeContext.path.concat([node.name]) : [node.name];
          const children = "children" in node && Array.isArray(node.children) ? await mapWithLimit(node.children, exportContext ? exportContext.maxConcurrency : 8, (child) => serializeNode(child, {
            parentId: node.id,
          ...
          result.parentId = Object.prototype.hasOwnProperty.call(nodeContext, "parentId") ? nodeContext.parentId : null;
          result.depth = typeof nodeContext.depth === "number" ? nodeContext.depth : 0;
          result.path = path;
          result.pathString = path.join(" / ");
          if (componentSource !== void 0) {
        ...
        async function serializeSelection(selection, options) {
          ...
          const filteredDocumentResources = clonedDocumentResources !== void 0 ? filterDocumentResources(clonedDocumentResources, exportContext, options || {}) : void 0;
          ...
          if (filteredDocumentResources !== void 0 || hasResolvedVariables) {
        ...
        function cssVariableName(name) {
          return String(name || "").trim().replace(/([a-z0-9])([A-Z])/g, "$1-$2").replace(/[^A-Za-z0-9]+/g, "-").replace(/^-|-$/g, "").toLowerCase();
        }
        ...
        function paintToCss(paint) {
          ...
          return paint.type ? paint.type.toLowerCase() : void 0;
        }
        ...
        function registerToken(map, order, prefix, value, payload) {
          ...
          if (!Object.prototype.hasOwnProperty.call(map, value)) {
            ...
            map[value] = {
              name,
              value: payload === void 0 ? value : payload
            };
            order.push(value);
          }
        ...
        function registerNamedToken(map, order, name, value) {
          if (!name || value === void 0 || value === null || value === "") {
            return void 0;
          }
        ...
        function buildAiInteractions(node) {
          ...
          return reactions.map((reaction) => {
            ...
            return Object.keys(interaction).length ? interaction : void 0;
          }).filter(Boolean);
        }
        ...
        async function serializeNodeForAi(node, context, options, aiContext) {
          ...
          const children = "children" in node && Array.isArray(node.children) ? await mapWithLimit(node.children, aiContext.maxConcurrency, (child) => serializeNodeForAi(child, {
        ...
        async function serializeSummaryNodeForAi(node, context, options, aiContext) {
          ...
          const componentSource = await readComponentSource(node);
          const componentId = registerComponentDefinition(componentSource, aiContext);
          const children = "children" in node && Array.isArray(node.children) ? await mapWithLimit(node.children, aiContext.maxConcurrency, (child) => serializeSummaryNodeForAi(child, {
            depth: (nodeContext.depth || 0) + 1
          }, options, aiContext)) : [];
          return omitDefaults({
            id: node.id,
        ...
        function buildFileName(selection) {
          const firstName = selection[0] && selection[0].name ? selection[0].name : "selection";
          const safeName = firstName.replace(/[\\/:*?"<>|]+/g, "-").replace(/\s+/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "selection";
        ...
        async function buildDocumentResources(figmaApi, exporter) {
          ...
          const variables = figmaApi.variables ? await callIfAvailable(figmaApi.variables, "getLocalVariablesAsync") : [];
          const variableCollections = figmaApi.variables ? await callIfAvailable(figmaApi.variables, "getLocalVariableCollectionsAsync") : [];
          return {
            styles: {
      ...
      (async function runPlugin() {
        ...
        if (!isPanel && !initialSelection.length) {
          ...
          }, postMcpStatus2 = function(message) {
            if (!isPanel || !uiReady) {
              return;
            }
            figma.ui.postMessage({
          ...
          }, buildPanelReadyMessage2 = function(metadata) {
            return {
              type: "panel-ready",
              selectionCount: currentSelection2().length,
          ...
          }, beginCurrentSelectionSummaryPush2 = function() {
            if (!isPanel || !uiReady) {
              return;
            }
            figma.ui.postMessage({
          ...
          }, scheduleMcpSummaryPush2 = function() {
            if (!isPanel) {
              return;
            }
            if (mcpDebounceTimer) {
          ...
          const resourceMode = figma.command === "export-full" ? "fullResources" : "referencedOnly";
          ...
          const resolveVariable = figma.variables && typeof figma.variables.getVariableByIdAsync === "function" ? async function(id) {
          ...
          const resolveVariableCollection = figma.variables && typeof figma.variables.getVariableCollectionByIdAsync === "function" ? async function(id) {
          ...
          async function buildExportOptions(mode) {
            const documentResources = mode === "ai-summary" ? null : await exporter.buildDocumentResources(figma, exporter);
            return {
          ...
          async function buildCurrentSelectionSummary() {
            const selection = currentSelection2();
            if (!selection.length) {
              postMcpStatus2("No selection");
              return null;
            }
          ...
          async function pushCurrentSelectionSummary() {
            ...
            try {
              ...
              figma.ui.postMessage({
                ...
                body: buildMcpEnvelope2(payload)
              });
              figma.ui.postMessage(buildPanelReadyMessage2(payload.metadata));
            } catch (error) {
              postMcpStatus2(`MCP summary failed: ${error instanceof Error ? error.message : String(error)}`);
            }
          }
          ...
          figma.ui.onmessage = (message) => {
            if (message && message.type === "ui-ready") {
              uiReady = true;
              postDownloadIfReady2();
              postPanelStateIfReady2();
              postMcpConfigIfReady2();
              return;
            }
          ...
          if (isPanel && typeof figma.on === "function") {
            try {
              figma.on("selectionchange", function() {
                scheduleMcpSummaryPush2();
              });
            } catch (error) {
              postMcpStatus2(`MCP listener failed: ${error instanceof Error ? error.message : String(error)}`);
            }
          }
          try {
            if (isPanel) {
              panelStateMessage = buildPanelReadyMessage2();
              postPanelStateIfReady2();
            } else {
